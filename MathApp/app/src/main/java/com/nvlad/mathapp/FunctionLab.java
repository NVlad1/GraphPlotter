...
public class FunctionLab implements Parcelable {
    ...
    private static int NMax = 300;
    private static int NMaxxx = 50000;
    ...
    FunctionLab () {
        NFunctions = 0;
        x = new double[NMaxxx];
        y = new double[NFunctionsMax][NMaxxx];
        IntersectX = new double[10*NMax];
        IntersectY = new double[10*NMax];
    ...
    public double Evaluator(int i, double x) throws ParseException, UnknownException{
    ...
    private double FindExtrema(double x01, double x02, int num, boolean ExT)throws ParseException, UnknownException{
        double x1=x01;
        double x2=x02;
        double x,y,x3,x4,y1,y2,y3,y4;
        double e = (xmax-xmin)/NMax;
        if (e>4e-5) e=4e-5;
...
//                                An[j].AddExtrema(x[i - 1], y[j][i - 1], true);
    ...
    public void CalculateRoots() throws ParseException,UnknownException,AnalysisException{
        if (!isEvalRoot) {
            for (int j = 0; j < NFunctions; j++) {
                An[j].CalculateRoots(xmin, xmax, y[j][0], y[j][NMax - 1], Eval[j]);
            }
            isEvalRoot = true;
        }
    }
            ...
            for (int j = 0; j < NFunctions - 1; j++)
                for (int k = j + 1; k < NFunctions; k++) {
                    if ((mode[j] == 0) & (mode[k] == 0)) {
                        ...
                        An_inter.CalculateRoots(xmin, xmax, y_temp[0], y_temp[NMax - 1], Eval_inter);
